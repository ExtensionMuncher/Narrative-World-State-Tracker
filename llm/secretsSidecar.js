...
RULES FOR charactersPresent (read carefully — this is the most error-prone field):
- Include ONLY characters who are physically in the scene and speaking, acting, or being acted upon RIGHT NOW.
...
- Be conservative. If you are unsure whether someone is actually present or just mentioned, LEAVE THEM OUT. A short, correct list is far better than a long, polluted one. Most scenes have 1-4 characters actually present.
...
- activePressures: only pressures actually active in the prose right now.
...
function buildSidecarPrompt(recentMessages, registry, secrets, userIdentity) {
    ...
    // User / player character identity — lets the analyzer reliably map the
    // [USER/PC] prose to a roster entity and distinguish the player from NPCs.
    if (userIdentity && (userIdentity.name || (userIdentity.aliases && userIdentity.aliases.length))) {
        prompt += '=== USER / PLAYER CHARACTER IDENTITY ===\n';
        prompt += `Name: ${userIdentity.name || '(not configured)'}\n`;
        prompt += `Aliases: ${(userIdentity.aliases || []).join(', ') || '(none)'}\n`;
        prompt += 'The [USER/PC] prose below is authored by/for this character. Treat them as present unless the prose clearly indicates otherwise.\n\n';
    }
...
function parseSidecarResponse(response, registry) {
    ...
    try {
        ...
        // Safety net: drop anything that resolves to a group/organization —
        // those are never "characters present" even if the model lists them.
        const validChars = [];
        if (Array.isArray(parsed.charactersPresent)) {
            for (const c of parsed.charactersPresent) {
                const resolved = registry.resolve(c);
                if (!resolved) continue;
                if (registry.isGroup && registry.isGroup(resolved)) continue;
                validChars.push(resolved);
            }
        }
...
export function getSceneContext(chatId) {
    ...
    if (sidecar) {
        ...
        if (sidecarFresh) {
            // Sidecar is AUTHORITATIVE for presence when fresh. The raw JS scan
            // flags any name string in the prose — including names that only
            // appear inside in-world documents (surveillance readouts, order
            // records, messages a character is reading). Those are mentions,
            // not presence. The sidecar LLM is instructed to exclude mentions,
            // so we trust its list rather than polluting it with the JS union.
            //
            // We keep the explicit user/PC if a user message is in range (the
            // sidecar may not always tag the silent PC), but we do NOT blindly
            // add every JS-detected name.
            const sidecarChars = new Set(sidecar.charactersPresent || []);
            if (userMessagePresent && userCanonical) sidecarChars.add(userCanonical);
            charactersPresent = Array.from(sidecarChars);

...
// East-Asian lunisolar layout support. Chromium/modern browsers expose the
// Unicode "chinese" calendar through Intl.DateTimeFormat; NWST uses that
// calendar only to obtain year-specific 29/30-day month layouts and
// intercalary-month placement. The roleplay's own configured month names remain
// the display authority. Results are cached in-memory because month math is a
// hot path during event/date calculations.
const LUNISOLAR_RAW_LAYOUT_CACHE = new Map();
const LUNISOLAR_MONTH_ORDINALS = {
    First: 1, Second: 2, Third: 3, Fourth: 4, Fifth: 5, Sixth: 6,
    Seventh: 7, Eighth: 8, Ninth: 9, Tenth: 10, Eleventh: 11, Twelfth: 12
};

/** True when Calendar Config is using NWST's dynamic East-Asian lunisolar engine. */
export function isLunisolarCalendar(calendarConfig) {
    return calendarConfig?.calendarSystem === 'lunisolar';
}

function utcDateWithFullYear(year, monthIndex = 0, day = 1) {
    const date = new Date(Date.UTC(2000, monthIndex, day));
    date.setUTCFullYear(year);
    return date;
}

function nextSignedYear(year) { return year === -1 ? 1 : year + 1; }

function intlChineseFormatter() {
    try {
        const fmt = new Intl.DateTimeFormat('en-u-ca-chinese', {
            calendar: 'chinese', year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC'
        });
        return fmt.resolvedOptions().calendar === 'chinese' ? fmt : null;
    } catch (_) {
        return null;
    }
}

function parseIntlLunarMonthName(value) {
    const text = String(value || '').trim();
    const match = text.match(/^(First|Second|Third|Fourth|Fifth|Sixth|Seventh|Eighth|Ninth|Tenth|Eleventh|Twelfth)\s+Month(?:\s*bis)?$/i);
    if (!match) return null;
    const ordinalKey = match[1][0].toUpperCase() + match[1].slice(1).toLowerCase();
    return {
        baseMonth: LUNISOLAR_MONTH_ORDINALS[ordinalKey],
        isLeapMonth: /bis/i.test(text)
    };
}

/**
 * Browser-backed East-Asian lunisolar year layout.
 * Each item is { baseMonth, isLeapMonth, days }. If Intl lacks the Chinese
 * calendar, a deterministic Metonic-style fallback keeps the calendar usable
 * (still 29/30-day months + occasional intercalary month) without pretending
 * to be an exact historical reconstruction.
 */
function rawLunisolarLayoutFor(year) {
    if (!Number.isInteger(year) || year === 0) year = 1;
    if (LUNISOLAR_RAW_LAYOUT_CACHE.has(year)) return LUNISOLAR_RAW_LAYOUT_CACHE.get(year);

    const fmt = intlChineseFormatter();
    if (fmt && year > 0 && year <= 9999) {
        try {
            const starts = [];
            const stopYear = nextSignedYear(year);
            const scanStart = utcDateWithFullYear(year, 0, 1).getTime();
            const scanEnd = utcDateWithFullYear(nextSignedYear(year), 3, 1).getTime();
            let firstNextYearStart = null;

            for (let t = scanStart; t <= scanEnd; t += 86400000) {
                const date = new Date(t);
                const parts = fmt.formatToParts(date);
                const relatedYear = Number(parts.find(p => p.type === 'relatedYear')?.value);
                const day = Number(parts.find(p => p.type === 'day')?.value);
                if (day !== 1 || !Number.isInteger(relatedYear)) continue;
                const monthText = parts.find(p => p.type === 'month')?.value || '';

                if (relatedYear === year) {
                    const parsed = parseIntlLunarMonthName(monthText);
                    starts.push({
                        time: t,
                        baseMonth: parsed?.baseMonth || null,
                        isLeapMonth: parsed?.isLeapMonth === true,
                        rawMonthName: monthText
                    });
                } else if (relatedYear === stopYear && starts.length > 0) {
                    firstNextYearStart = t;
                    break;
                }
            }

            if (starts.length >= 12 && firstNextYearStart != null) {
                // Defensive fallback for unusual Intl month-label shapes: infer
                // missing base month numbers from sequence, preserving a repeated
                // month as the intercalary occurrence when possible.
                let lastBase = 0;
                for (const entry of starts) {
                    if (!Number.isInteger(entry.baseMonth)) {
                        entry.baseMonth = Math.min(12, lastBase + (entry.isLeapMonth ? 0 : 1));
                    }
                    lastBase = entry.baseMonth || lastBase;
                }

                const layout = starts.map((entry, index) => {
                    const nextTime = index + 1 < starts.length ? starts[index + 1].time : firstNextYearStart;
                    return {
                        baseMonth: entry.baseMonth,
                        isLeapMonth: entry.isLeapMonth,
                        days: Math.round((nextTime - entry.time) / 86400000),
                        source: 'intl-chinese'
                    };
                }).filter(entry => entry.days === 29 || entry.days === 30);

                if (layout.length >= 12) {
                    LUNISOLAR_RAW_LAYOUT_CACHE.set(year, layout);
                    return layout;
                }
            }
        } catch (error) {
            console.warn('[NWST Calendar] Intl Chinese-calendar layout failed; using lunisolar fallback:', error);
        }
    }

    // Fallback: 19-year cycle with seven intercalary years. This is structural,
    // not a claim of historical Chinese/Japanese calendrical exactness.
    const cycleYear = ((Math.abs(year) - 1) % 19) + 1;
    const leap = new Set([3, 6, 8, 11, 14, 17, 19]).has(cycleYear);
    const layout = [];
    for (let baseMonth = 1; baseMonth <= 12; baseMonth++) {
        layout.push({ baseMonth, isLeapMonth: false, days: baseMonth % 2 === 1 ? 30 : 29, source: 'fallback-metonic' });
        if (leap && baseMonth === 6) {
            layout.push({ baseMonth, isLeapMonth: true, days: 29, source: 'fallback-metonic' });
        }
    }
    LUNISOLAR_RAW_LAYOUT_CACHE.set(year, layout);
    return layout;
...
function lunisolarLeapMonthLabel(baseName, calendarConfig) {
    const template = typeof calendarConfig?.lunisolar?.leapMonthLabel === 'string' && calendarConfig.lunisolar.leapMonthLabel.trim()
        ? calendarConfig.lunisolar.leapMonthLabel.trim()
        : 'Intercalary {month}';
    return template.replace(/\{month\}/gi, baseName);
}

/**
 * Full month layout for one calendar year. In standard mode each configured
 * month appears once. In lunisolar mode a year may contain 12 or 13 layout
 * entries; leap entries repeat the underlying base month with a distinct label.
 */
export function calendarMonthLayoutFor(calendarConfig, year = 1) {
    const baseNames = (calendarConfig?.enabled && Array.isArray(calendarConfig.monthNames) && calendarConfig.monthNames.length)
        ? calendarConfig.monthNames
        : GREGORIAN_MONTH_NAMES;

    if (!isLunisolarCalendar(calendarConfig)) {
        const lengths = standardMonthLengthsFor(calendarConfig, year);
        return lengths.map((days, index) => ({
            baseMonth: index + 1,
            isLeapMonth: false,
            days: Number(days) || 30,
            name: String(baseNames[index] || `Month ${index + 1}`),
            source: 'configured'
        }));
    }

    return rawLunisolarLayoutFor(year).map(entry => {
        const baseName = String(baseNames[entry.baseMonth - 1] || `Month ${entry.baseMonth}`);
        return {
            ...entry,
            name: entry.isLeapMonth ? lunisolarLeapMonthLabel(baseName, calendarConfig) : baseName
        };
    });
}

/** Find a concrete year-layout date from a configured/base month number. */
export function calendarDateForBaseMonthDay(year, baseMonth, day, calendarConfig, preferLeap = false) {
    const layout = calendarMonthLayoutFor(calendarConfig, year);
    const target = layout.findIndex(entry => entry.baseMonth === Number(baseMonth) && entry.isLeapMonth === !!preferLeap);
    if (target < 0) return null;
    const d = Number(day);
    if (!Number.isInteger(d) || d < 1 || d > layout[target].days) return null;
    return { year, month: target + 1, day: d };
}

/** Metadata for an internal concrete date (whose month is a year-layout index). */
export function calendarMonthInfoForDate(date, calendarConfig) {
    if (!date || !Number.isInteger(date.year)) return null;
    return calendarMonthLayoutFor(calendarConfig, date.year)[Math.max(0, Number(date.month || 1) - 1)] || null;
}
...
/**
 * Whether the active calendar still maps cleanly onto Gregorian month/day
 * coordinates. Renaming months or weekdays does NOT make a calendar
 * structurally non-Gregorian; changing the month count or any month length
 * does. This is shared by features (leap handling, real-world holidays) that
 * care about calendar shape rather than display labels.
 *
 * The default calendar is Gregorian by definition. For custom calendars we
 * require the normal 12 Gregorian month lengths and a 7-day week, but the
 * names themselves may be anything.
 *
 * @param {object} calendarConfig
 * @returns {boolean}
 */
export function isGregorianCompatibleCalendar(calendarConfig) {
    if (isLunisolarCalendar(calendarConfig)) return false;
    if (!calendarConfig || calendarConfig.enabled !== true) return true;

    const monthDays = Array.isArray(calendarConfig.monthDays) ? calendarConfig.monthDays : [];
    const weekDays = Array.isArray(calendarConfig.weekDays) ? calendarConfig.weekDays : [];
    if (monthDays.length !== GREGORIAN_MONTH_DAYS.length || weekDays.length !== 7) return false;

    return GREGORIAN_MONTH_DAYS.every((days, index) => Number(monthDays[index]) === days);
}

function standardMonthLengthsFor(calendarConfig, year) {
    if (calendarConfig?.enabled && Array.isArray(calendarConfig.monthDays) && calendarConfig.monthDays.length > 0) {
        // Gregorian-compatible custom calendars may rename their month and
        // weekday labels while preserving the real 12-month/day structure.
        // Those calendars honor the leap toggle; structurally different
        // calendars use exactly their configured month lengths.
        if (calendarConfig.leapYears !== false
            && isGregorianCompatibleCalendar(calendarConfig)
...
/**
 * Month lengths for a given year under the active calendar. Lunisolar mode
 * returns the concrete 12/13-month layout for that year.
 */
export function monthLengthsFor(calendarConfig, year) {
    if (isLunisolarCalendar(calendarConfig)) {
        return calendarMonthLayoutFor(calendarConfig, year).map(entry => entry.days);
    }
    return standardMonthLengthsFor(calendarConfig, year);
}

/**
 * Month names under the active calendar. Pass a year to receive that year's
 * concrete lunisolar layout names (including an intercalary month when present).
 */
export function monthNamesFor(calendarConfig, year = null) {
    if (isLunisolarCalendar(calendarConfig) && Number.isInteger(year) && year !== 0) {
        return calendarMonthLayoutFor(calendarConfig, year).map(entry => entry.name);
    }
...
/** Matching aliases for display-oriented month labels that include decorative
 * trailing punctuation (e.g. "Shimotsuki, the Eleventh Month,"). The saved
 * label remains untouched; parsing merely tolerates the LLM omitting the final
 * comma/period when it writes the date. */
function monthNameAliases(name) {
    const exact = String(name || '').trim();
    if (!exact) return [];
    const stripped = exact.replace(/[\s,.;:·]+$/u, '').trim();
...
export function computeDeterministicDate(startDate, anchorDayCount, dayCount, calendarConfig) {
    ...
    const monthNames = monthNamesFor(calendarConfig, date.year);
...
export function parseDisplayDate(display, calendarConfig) {
    ...
    // "Duskmonth Deep" wins over "Duskmonth"). Lunisolar mode resolves
    // the concrete year's intercalary-month label before matching.
    let bc = false;
    const bcMatch = s.match(/\s+(BC|BCE)\.?$/i);
    if (bcMatch) { bc = true; s = s.slice(0, bcMatch.index).trim(); }
    const inlineYearMatch = s.match(/(\d{1,6})\s*$/);
    const inlineYear = inlineYearMatch ? (bc ? -parseInt(inlineYearMatch[1], 10) : parseInt(inlineYearMatch[1], 10)) : null;
    const names = monthNamesFor(calendarConfig, Number.isInteger(inlineYear) && inlineYear !== 0 ? inlineYear : null)
        .flatMap((n, i) => monthNameAliases(n).map(alias => ({ n: alias, i })))
        .filter(x => x.n)
    ...
    for (const { n, i } of names) {
        ...
        if (m) {
            ...
            const signedYear = bc ? -year : year;
            const lengths = monthLengthsFor(calendarConfig, signedYear);
...
export function parseUserDate(text, dmy = false, calendarConfig = null) {
    ...
    const finish = (year, month, day) => {
        ...
        const signedYear = bc ? -Math.abs(year) : year;

        if (isLunisolarCalendar(calendarConfig)) {
            // User-entered numeric dates stay intuitive: month 8 means the
            // regular eighth month even if an intercalary month occurs earlier.
            return calendarDateForBaseMonthDay(signedYear, month, day, calendarConfig, false);
        }
        ...
        return { year: signedYear, month, day };
    };
...
export function parseCurrentCalendarDate(dateDisplay, dateSub, calendarConfig, dmy = false) {
    ...
    const names = monthNamesFor(calendarConfig, year)
        .flatMap((name, index) => monthNameAliases(name).map(alias => ({ name: alias, index })))
        .filter(x => x.name)
    ...
    if (numeric) {
        ...
        const day = dmy ? a : b;
        if (isLunisolarCalendar(calendarConfig)) {
            return calendarDateForBaseMonthDay(year, month, day, calendarConfig, false);
        }
        const lengths = monthLengthsFor(calendarConfig, year);
